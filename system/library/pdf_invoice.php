	...
	protected function AddOrderTable()
	{
        ...
        if ($this->data['order']['products']) {
            ...
            foreach ($this->data['order']['products'] as $product) {
                //dicky
                $pre_order = "";
                if ($product['last_stock'] != null){
                    if($product['last_stock'] <= 0){

                        $pre_order = '<div style="color:red">Pre-Order</div>';

                    }else if($product['last_stock'] - $product['quantity'] < 0){
    
                            $less_product = abs($product['last_stock'] - $product['quantity']);
                            $pre_order = '<div style="color:red">('. $less_product  .') quantity to be back ordered.<br>Arrival date will be advised over email.</div>';
                ...
                $row_data = array(
                    '<a href="' . $product['url'] . '" style="text-decoration:none; color:#000000;">' . $product['name'] . '</a>' . $product['option'] . $pre_order,
