...
$this->load->model('extension/custverf');
			if($this->model_extension_custverf->getmodstatus() && isset($this->request->get['route']) && stristr($this->request->get['route'], 'customer/customer') && isset($this->session->data['custverf_customer_id']) && $this->session->data['custverf_customer_id']) {
				$this->document->addScript('view/javascript/custverf.js');
			}
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$filter_custverf = (isset($this->request->get['filter_custverf'])) ? $this->request->get['filter_custverf'] : null; 
			$url = '';
			if (isset($this->request->get['filter_custverf'])) { $url .= '&filter_custverf=' . $this->request->get['filter_custverf']; } 
...
'filter_custverf' => $filter_custverf,
		...
		foreach ($results as $result) {
			...
			$data['customers'][] = array(
				...
				'name'           => $result['name'],
'custverf_confirm' => (int)$result['custverf_confirm'],
			'sendverifyemail' => $this->url->link('extension/module/custverf/sendverifyemail', 'user_token=' . $this->session->data['user_token'] . '&customer_id=' . $result['customer_id'], true),
				'email'          => $result['email'],
...
$data['sort_custverf_confirm'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . '&sort=c.custverf_confirm' . $url, 'SSL');
...
$data['module_custverf_status'] = $this->config->get('module_custverf_status');
			$data['column_custverf_confirm'] = $this->language->get('column_custverf_confirm') != 'column_custverf_confirm' ? $this->language->get('column_custverf_confirm') : 'Email Verified ?';
			$data['text_yes'] = $this->language->get('text_yes') != 'text_yes' ? $this->language->get('text_yes') : 'Yes';
			$data['text_no'] = $this->language->get('text_no') != 'text_no' ? $this->language->get('text_no') : 'No';
			$data['button_sendverifyemail'] = $this->language->get('button_sendverifyemail') != 'button_sendverifyemail' ? $this->language->get('button_sendverifyemail') : 'Send Verification Email Again ?';
			$data['filter_custverf'] = $filter_custverf;
