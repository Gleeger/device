...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		if (isset($data['product_option'])) {
			foreach ($data['product_option'] as $product_option) {
				...
				if (!empty($product_option_value['special'])) {
					foreach ($product_option_value['special'] as $option_special) {
						$this->db->query("INSERT INTO " . DB_PREFIX . "option_special SET product_id = '" . (int)$product_id . "', product_option_id = '" . (int)$product_option_id . "', product_option_value_id = '" . (int)$product_option_value_id . "', customer_group_id = '" . (int)$option_special['customer_group_id'] . "', priority = '" . (int)$option_special['priority'] . "', price = '" . (float)$option_special['price'] . "', date_start = '" . $this->db->escape($option_special['date_start']) . "', date_end = '" . $this->db->escape($option_special['date_end']) . "'");
					}
				}
	...
	public function getProducts($data = array()) {
                ...
                if (isset($data['filter_category']) && $data['filter_category'] !== '') {
                    $sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p.product_id = p2c.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p2c.category_id = '" . (int)$data['filter_category'] . "'";
				}
				...
				if (isset($data['filter_manufacture']) && $data['filter_manufacture'] !== '') {
					$sql .= " AND p.manufacturer_id = '" . (int)$data['filter_manufacture'] . "'";
	...
	public function getProductDiscounts($product_id) {
		...
		return $query->rows;
				...
				public function getProductOptionValueSpecials($product_option_value_id) {
					$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "option_special WHERE product_option_value_id = '" . (int)$product_option_value_id . "' ORDER BY priority, price");
	...
	public function getTotalProducts($data = array()) {
		$sql = "SELECT COUNT(DISTINCT p.product_id) AS total FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)";
				...
				if (isset($data['filter_manufacture']) && $data['filter_manufacture'] !== '') {
					$sql .= " AND p.manufacturer_id = '" . $this->db->escape($data['filter_manufacture']) . "'";
