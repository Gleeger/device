		...
		if ($product_info) {
			...
			foreach ($product_option_values as $product_option_value) {
				...
				if ($this->config->get('module_option_special_status')) {
					$option_special_query = $this->db->query("SELECT price FROM " . DB_PREFIX . "option_special WHERE product_option_value_id = '" . (int)$product_option_value['product_option_value_id'] . "' AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((date_start = '0000-00-00' OR date_start < NOW()) AND (date_end = '0000-00-00' OR date_end > NOW())) ORDER BY priority ASC, price ASC LIMIT 1");

					if ($option_special_query->num_rows) {
						$product_option_value['price'] = $option_special_query->row['price'];
					}
				}
