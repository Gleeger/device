	...
	private function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
			    ...
			    //dicky update preorder - save last stock
                $last_stock_query = $this->db->query("SELECT quantity FROM " . DB_PREFIX . "product WHERE product_id = '" .(int)$product['product_id'] . "'");
                $last_stock = -1;
                if ($last_stock_query->num_rows) {
                        $last_stock = $last_stock_query->row['quantity'];
                }
                //dicky update preorder - save last stock END
                
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET last_stock = '" . (int)$last_stock . "', order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "'");
